...
class GraphModule(nn.Module):
    def __init__(self, in_size, out_size, num_layers, num_proposals, feat_size, num_locals, 
        query_mode="corner", graph_mode="edge_conv", return_edge=False, graph_aggr="add", 
        ...
        # map input
        self.map_input = nn.Linear(in_size, out_size)
        ...
        for _ in range(num_layers):
            if graph_mode == "graph_conv":
                self.gc_layers.append(GCNConv(out_size, out_size))
            elif graph_mode == "edge_conv":
                self.gc_layers.append(EdgeConv(out_size, out_size, graph_aggr))
            else:
        ...
        if self.return_orientation: 
            ...
            self.edge_layer = EdgeConv(out_size, out_size, graph_aggr)
    ...
    def _query_locals(self, det_dict, target_ids, object_masks, include_self=True, overlay_threshold=0.5):
        corners = det_dict["proposal_bbox_batched"] # batch_size, num_proposals, 8, 3
        ...
        self_masks = torch.zeros(batch_size, self.num_proposals).type_as(target_ids)
        ...
        local_masks = torch.zeros(batch_size, self.num_proposals).type_as(target_ids)
        local_masks.scatter_(1, topk_ids, 1)

        return local_masks

    def _create_adjacent_mat(self, det_dict, object_masks):
        ...
        for obj_id in range(num_objects):
            target_ids = torch.tensor([obj_id for _ in range(batch_size)]).type_as(object_masks).long()
            adjacent_entry = self._query_locals(det_dict, target_ids, object_masks, include_self=False) # batch_size, num_objects
    ...
    def forward(self, data_dict):
        obj_feats = data_dict["proposal_feats_batched"] # batch_size, num_proposals, in_size
        object_masks = data_dict["proposal_batch_mask"] # batch_size, num_proposals

        obj_feats = self.map_input(obj_feats) # batch_size, num_proposals, out_size
        ...
        new_obj_feats = torch.zeros(batch_size, num_objects, self.feat_size).type_as(obj_feats)
        edge_indices = torch.zeros(batch_size, 2, num_objects * self.num_locals).type_as(obj_feats)
        edge_feats = torch.zeros(batch_size, num_objects, self.num_locals, self.out_size).type_as(obj_feats)
        edge_preds = torch.zeros(batch_size, num_objects * self.num_locals, self.num_bins+1).type_as(obj_feats)
        num_sources = torch.zeros(batch_size).type_as(obj_feats).long()
        num_targets = torch.zeros(batch_size).type_as(obj_feats).long()
        for batch_id in range(batch_size):
            ...
            batch_graph = GData(x=batch_obj_feats, edge_index=batch_edge_index.type_as(obj_feats).long())
            ...
            if self.return_orientation:
                ...
                try:
                    ...
                    _, edge_feat = self.edge_layer(node_feat, batch_edge_index.type_as(obj_feats).long())
