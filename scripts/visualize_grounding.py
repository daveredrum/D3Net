...
def load_data(args):
    pred_path = os.path.join(OUTPUT_ROOT, args.folder, "predictions.p")
...
def visualize(args):
    ...
    root = os.path.join(OUTPUT_ROOT, args.folder, "vis_grounding")
    os.makedirs(root, exist_ok=True)

    for scene_id in tqdm(predictions.keys()):
        scene_root = os.path.join(root, scene_id)
        os.makedirs(scene_root, exist_ok=True)

        # print("exporting axis-aligned ScanNet meshes...")
        # visualize_aligned_mesh(scene_id, scene_root)

        # print("exporting axis-aligned high-res ScanNet meshes...")
        # visualize_aligned_high_mesh(scene_id, scene_root)

        for object_id in predictions[scene_id]:
            ann_ids = list(predictions[scene_id][object_id].keys())
            gt_bbox = predictions[scene_id][object_id][ann_ids[0]]["gt_bbox"]
            gt_path = os.path.join(scene_root, "gt-{}.ply".format(object_id))
            write_bbox(gt_bbox, 0, gt_path)
            
            for ann_id in ann_ids:
                pred_bbox = predictions[scene_id][object_id][ann_ids[0]]["pred_bbox"]
                pred_path = os.path.join(scene_root, "pred-{}-{}.ply".format(object_id, ann_id))
